...
**测试日期：** 2026-03-25
**测试方式：** 内部记录信号（UP/DOWN 双通道）全步进统计
**数据文件时间戳：** 2026-03-25 09:06:59

### 数据信息

- 扫描范围：Phase Step 0~224（共 225 点）
- 每个相位点配对样本数：3000
- 统计数据文件：`full_phase_stats_0_224_20260325_094752.csv`
- 输出四子图：`full_phase_stats_0_224_20260325_094752.png`

#### 关键统计汇总

| 指标                        | UP 通道            | DOWN 通道          |
| --------------------------- | ------------------ | ------------------ |
| 均值范围（Phase 0~224）     | 36.733~3805.460 ps | 31.414~3808.775 ps |
| 标准差均值（过滤异常点）    | 13.064 ps          | 12.283 ps          |
| 过滤异常点（Phase 200~205） | 6个                | 6个                |

#### 数据分析图

**UP/DOWN 均值与标准差（按 Phase Step）**

![第三次测试四子图](tdc_results/full_phase_stats_0_224_20260325_094752.png)

- 四子图图表已生成，展示 Phase 0~224 全范围数据
- 上图：UP/DOWN 通道均值对比（线性趋势）
- 下图：UP/DOWN 通道标准差对比（异常点已标注）
...
**测试日期：** 2026-03-25
**测试方式：** UART 全扫描（UP/DOWN 同步记录）
**原始数据文件：** `tdc_uart_全扫描_both_20260325_090659.txt`

### 数据信息

- 扫描步进：0~224（共 225 点）
- 每步记录：UP 1 点 + DOWN 1 点
- 总记录数：450 条（UP 225 条，DOWN 225 条）
- 综合分析图：`tdc_uart_全扫描_both_20260325_090659.png`

#### 关键统计汇总

| 指标                       | UP 通道             | DOWN 通道           |
| -------------------------- | ------------------- | ------------------- |
| Fine 范围                  | 6~3822              | 18~3840             |
| Coarse 范围                | 116~117             | 138~139             |
| 截断后线性拟合斜率（图中） | ~0.997 ps/ps        | ~0.998 ps/ps        |
| 线性度指标（图中）         | DNL RMS = 0.887 LSB | INL RMS = 0.777 LSB |

#### 数据分析图

**全扫描综合分析（Fine/Coarse、线性拟合、DNL/INL）**

![全扫描综合分析图](tdc_results/tdc_uart_全扫描_both_20260325_090659.png)

**图中结论（关键点）：**

- UP/DOWN Fine 曲线在主区间基本线性，约在 Phase 200 附近出现翻转（计数回绕）
- Coarse 计数稳定在窄范围内（UP: 116-117，DOWN: 138-139）
- 截断后拟合曲线显示 UP/DOWN 斜率一致性较好
- DNL/INL 均处于较小 RMS 区间，整体线性度表现稳定
